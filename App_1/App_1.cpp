...
void run_test(size_t size, int32_t thread_count, double &time_multithread, double &time_singlethread);
...
void fn_th_mul(int32_t *begin, int32_t *end, int32_t *ret_val) {
  int64_t result = 1;
  for (; begin != end; begin++) 
    result = ((int64_t)*begin * result) % mod;
  *ret_val = (int32_t)result;
}
...
void run_test(size_t size, int32_t thread_count, double &time_multithread, double &time_singlethread) {
  ...
  } a(size), sub_results(thread_count);
  ...
  for (auto &th : threads) {
    if (i + 1 == thread_count)
      th = thread(fn_th_mul, a.data + shift, a.data + size, sub_results.data + i);
    else
      th = thread(fn_th_mul, a.data + shift, a.data + shift + step, sub_results.data + i);
    i++;
    shift += step;
  }
  ...
  for (int i = 1; i < thread_count; i++)
    sub_results.data[0] = (uint64_t)sub_results.data[i] * sub_results.data[0] % mod;
