...
// функция умножения для потока
void mul_f(matrix &result, matrix &a, matrix &b, int from, int to) {
...
matrix mul_th(matrix& a, matrix& b,uint32_t thread_count) {
  ...
  if (b.width() < info.threads) {
    // потоков больше чем столбцов
    for (uint16_t i = 0; i != b.width(); i++)
      threads[i] = thread(mul_f, std::ref(result), std::ref(a), std::ref(b), (int)i, (int)i+1);
    for (uint16_t i = 0; i != b.width(); i++)
      threads[i].join();
  }
  else {
    int step = b.width() / thread_count;
    uint32_t i = 0;
    ...
    for (auto &th : threads) {
      ...
      th = thread(mul_f, std::ref(result), std::ref(a), std::ref(b), from, to);
      i++;
    }
